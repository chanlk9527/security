...
Bean Validation 很适合回答“这个对象是否符合业务契约”，但现代应用还经常把输入交给另一种解释器：数据库把字符串解释成 SQL，操作系统解释成命令，浏览器解释成 HTML 和 JavaScript，模板引擎解释表达式，反序列化框架甚至可能把数据解释成要实例化的类型。许多注入漏洞的共同根源，正是程序没有清楚区分“数据”和“指令”。

解决这类问题的首选方法不是写出更复杂的正则表达式，而是避免让数据参与生成指令。SQL 使用参数化查询，系统命令使用参数数组并尽量取消 Shell，模板只接受受控变量，反序列化只允许预期的数据类型。输出编码则必须跟着目标上下文变化：放进 HTML 文本、属性、JavaScript、CSS 和 URL 的同一段字符串，需要采用不同的处理方式。富文本确实允许一部分标记，此时要使用成熟的允许列表净化器，并用内容安全策略限制遗漏造成的影响。

### 当输入不是一个字段，而是一个地址

假设系统提供“输入图片 URL，服务器代为下载”的功能。开发者可能会认为，服务器只是发出一次普通 HTTP 请求，最多遇到下载失败。可是在云环境中，服务器能访问的地址往往比用户多得多：本机管理端口、数据库、集群控制面，甚至能够返回临时云凭证的元数据服务。攻击者让服务器替自己访问这些位置，就形成了服务端请求伪造，即 SSRF。

SSRF 的关键不是验证 URL 长得像不像网址，而是限制服务器最终能够连接到哪里。最稳妥的办法是只允许业务确实需要的目标；无法建立允许列表时，也要限制协议、端口、重定向、响应大小和超时，阻止本机、内网、云元数据等受保护地址。仅在解析域名时检查一次还不够，因为攻击者可以让同一域名在检查和连接时返回不同地址。高风险下载服务通常通过独立出口代理发起请求，由代理统一执行地址策略和审计。

### 一张图片为什么需要隔离解析

文件上传把三个问题叠在了一起。首先，文件名可能造成路径穿越或覆盖；其次，浏览器和服务端可能把文件当成可执行内容；最后，图片、视频、压缩包和办公文档都要交给复杂解析器，而解析器本身经常出现内存破坏、递归解压和资源耗尽漏洞。

因此，文件扩展名和客户端提供的 `Content-Type` 只能当作提示，服务端还要检查实际结构、大小和允许的格式。对象应使用随机内部名称存放在非执行区域，通过独立域名或附件方式提供下载。压缩包要限制层数和解压后的总大小，转码与文档解析则放在资源、网络和权限受限的隔离进程中。这样即使某个解析库仍有未知漏洞，攻击者拿到的也只是一个狭小、可丢弃的执行环境，而不是整个业务服务。
